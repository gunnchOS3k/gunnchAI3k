...
export function createWave003Context(cwd = process.cwd()): Wave003Context {
  ...
  const scratchRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'gunnchai-wave003-'));
  const resultsDir = path.join(repoRoot, 'evals', 'wave003', 'results');
  const evidenceDir = path.join(repoRoot, 'evidence', 'engineering_wave003');
