...
Secondary (Pixel local not feasible): `NEXT_GUNNCHAI_ACTION=KEEP_PIXEL_AS_CLIENT_AND_QUALIFY_NEARBY_EDGE_EXECUTION`
...
### D — Pixel preflight
`artifacts/kirby_v2/live/pixel6a/PIXEL_BASELINE.json` — **`PIXEL6A_ADB_CONNECTED=true`**, classification `PIXEL_READY` (bluejay). Full getprop/meminfo/df/battery/thermalservice suite captured. Prior `PIXEL_ADB_BLOCKED` cleared.
...
`docs/frontier/PIXEL6A_INFERENCE_RUNTIME_DECISION.md` + `RUNTIME_DECISION.json` — **`RUNTIME_NEARBY_EDGE_MAC` preferred**; `RUNTIME_ON_DEVICE_LOCAL=false` (no on-device binary).

### P — PIXEL6A_LIVE_LOCAL_MODEL_PASS
**FAIL (honest)** — ADB connected, but **no on-device runtime** (no llama.cpp/ollama/Termux+GGUF). ADB-forwarded Mac inference **not** claimed as LIVE_PIXEL.

### Q — Android client
`GUNNCHAI_ANDROID_CLIENT_NOT_AVAILABLE` — no Android app tree in repo.

### R — DEVICE_EDGE_ROUTING_PROVENANCE_PASS
**PASS** — compute=Mac `LIVE_MAC`; device_role=`pixel6a_adb_client`; Pixel-local evidence `UNAVAILABLE`.

### S — Thermal / energy
20 short Mac completions **PASS**; Pixel thermal snapshot status **0 (NONE)**, battery ~20% AC, no on-device model load; 5-minute soak skipped.
...
| PIXEL6A_ADB_CONNECTED | PASS |
| PIXEL6A_LIVE_LOCAL_MODEL_PASS | FAIL (ADB_OK_NO_ON_DEVICE_RUNTIME) |
| DEVICE_EDGE_ROUTING_PROVENANCE_PASS | PASS (Mac compute / Pixel ADB client) |
